...
package com.bloomreach.brxdemos.pacificsupply.translate.api.feed;
...
public class ProcessedFeed {
    ...
    private final static int PRODNAME_RECORD_INDEX = 405;   // === title
    ...
    // given a pid, return true if given view is associated with it in the catalog
    // This is necessary to check when selecting a random PID to simulate pixelLog
    public boolean isViewAssociatedWithPid (String pid, String requiredViewId) {
        for (FeedRecord fr : processedFeedList) {
            if (fr.getProductId ().equals (pid) == true) {
                String[] viewIdList;

                viewIdList = fr.getViewIdList ();
                for (int i = 0; i < viewIdList.length; i++) {
                    if (viewIdList[i].equals (requiredViewId) == true)
                        return (true);
                }
            }
        }

        // given pid is not associated with the required viewId
        return (false);
    }
        ...
        try {       
            ...
            while ((srcLine = srcBufferedReader.readLine ()) != null) {
                ...
                if (tokens != null) {
                    ...
                    String prodName = tokens [PRODNAME_RECORD_INDEX].trim ();

                    feedRec = new FeedRecord (prodId, prodName, priceList, fullCrumb, fullCrumbId, viewIdList);
    ...
    class FeedRecord {
        ...
        FeedRecord (String prodId, String prodName, String productPrices, String fullCrumb, String fullCrumbId, String viewIds) {
