...
public class CuratedJourneyGenerator {
    ...
    private final static int CURATEDTERM_SEARCH_SESSION_WEIGHT = 20;
    ...
    public CuratedJourneyGenerator () {
    }
    ...
    public void startJourney (UserRecord userRecord,
        ...
        for (int sessionNum = 0; sessionNum < numSessions; sessionNum++) { 
            ...
            MessageLogger.logDebug (String.format ("Curated journey, end session: userId = %s, sessionNum = %d", userRecord.getUserId(),  sessionNum));
            newSessionLog.close ();
    ...
    private StepResult startOneSession (StepResult prevStepResult,
        ...
        switch (sessionType) {
            case GeneratorConstants.TERM_SEARCH_SESSION:
                MessageLogger.logDebug ("Curated journey, generate search query session, userId = " + userRecord.getUserId()); 
            ...
            case GeneratorConstants.CATEGORY_SEARCH_SESSION: 
                MessageLogger.logDebug ("Curated journey, generate category query session, userId = " + userRecord.getUserId()); 
            ...
            case GeneratorConstants.ZERO_RESULT_QUERY_SESSION:
                MessageLogger.logDebug ("Curated journey, generate zeroQuery session, userId = " + userRecord.getUserId()); 
            ...
            case GeneratorConstants.TERM_SEARCH_WITH_TERM_REFINEMENT_SESSION: 
                MessageLogger.logDebug ("Curated journey, generate searchquery + searchquery refined session, userId = " + userRecord.getUserId()); 
            ...
            case GeneratorConstants.TERM_SEARCH_WITH_CATEGORY_REFINEMENT_SESSION:
                MessageLogger.logDebug ("Curated journey, generate searchquery + category refined session, userId = " + userRecord.getUserId()); 
            ...
            case GeneratorConstants.CATEGORY_SEARCH_WITH_TERM_REFINEMENT_SESSION:
                MessageLogger.logDebug ("Curated journey, generate category + searchquery refined session, userId = " + userRecord.getUserId()); 
            ...
            case GeneratorConstants.CATEGORY_SEARCH_WITH_CATEGORY_REFINEMENT_SESSION:
                MessageLogger.logDebug ("Curated journey, generate category + category refined session, userId = " + userRecord.getUserId()); 
            ...
            case GeneratorConstants.ZERO_RESULT_QUERY_WITH_TERM_REFINEMENT_SESSION:
                MessageLogger.logDebug ("DELETED Curated journey, generate zeroQuery + search refined session, userId = " + userRecord.getUserId()); 
            ...
            case GeneratorConstants.ZERO_RESULT_QUERY_WITH_CATEGORY_REFINEMENT_SESSION:
                MessageLogger.logDebug ("DELETED Curated journey, generate zeroQuery + category refined session, userId = " + userRecord.getUserId()); 
            ...
            case GeneratorConstants.SUGGEST_SESSION_SELECT_NONE:
                MessageLogger.logDebug ("Curated journey, generate suggest session, userId = " + userRecord.getUserId()); 
            ...
            case GeneratorConstants.TERM_PARTIAL_SEARCH_WITH_TERM_REFINEMENT_SESSION:
                MessageLogger.logDebug ("Curated journey, generate partial s2s session, userId = " + userRecord.getUserId()); 
            ...
            case GeneratorConstants.TERM_PARTIAL_SEARCH_WITH_CATEGORY_REFINEMENT_SESSION:
                MessageLogger.logDebug ("Curated journey, generate partial s2c session, userId = " + userRecord.getUserId()); 
            ...
            case GeneratorConstants.CATEGORY_PARTIAL_SEARCH_WITH_TERM_REFINEMENT_SESSION:
                MessageLogger.logDebug ("Curated journey, generate partial c2s session, userId = " + userRecord.getUserId()); 
            ...
            case GeneratorConstants.CATEGORY_PARTIAL_SEARCH_WITH_CATEGORY_REFINEMENT_SESSION:
                MessageLogger.logDebug ("Curated journey, generate partial c2c session, userId = " + userRecord.getUserId()); 
    ...
    private StepResult startSearchSession (StepResult prevStepResult,
        ...
        stepLog.setUserSegment (userRecord.getSegment());
        stepResult = this.stepsHandler.handleStepCuratedSearchTerm (prevStepResult, userRecord, logTime, stepLog);
        if (stepResult instanceof StepResultInvalidData) {
    ...
    private StepResult startSearch2SearchRefinedQuerySession (StepResult prevStepResult,
                                                              ...
                                                              Cart userCart) throws Exception {
        StepResult stepResult;
        CuratedSearchTermDetails selectedSearchTerm;
        ...
        stepLog = sessionLog.addStepLog (GeneratorConstants.TRAFFIC_STEPID_SEARCH_TERM, logTime);
        stepLog.setUserSegment (userRecord.getSegment());
        selectedSearchTerm = this.curatedSearchTerms.selectSearchTermAtRandom (prevStepResult.getUrl());
        stepResult = this.stepsHandler.handleStepSearchTerm (prevStepResult, userRecord, logTime, 
                                                             stepLog, selectedSearchTerm.getInitialQuery ());
        if (stepResult instanceof StepResultInvalidData) {
            MessageLogger.logWarning ((String) stepResult.getData ());
            stepLog.setStepResult ((String) stepResult.getData ());
            return stepResult;
        ...
        stepLog = sessionLog.addStepLog (GeneratorConstants.TRAFFIC_STEPID_SEARCH_TERM, logTime);
        stepLog.setUserSegment (userRecord.getSegment());
        refinedSearchTerm = this.curatedSearchTerms.selectRefinedSearchTermAtRandom (selectedSearchTerm);
    ...
    private StepResult startCategory2SearchRefinedQuerySession (StepResult prevStepResult,
        ...
        StepResult stepResult;
        CuratedSearchTermDetails selectedRefinedSearchTerm;
        StepResultProductDetails stepResultProductDetails;
        long logTime;
        StepLog stepLog;
        ...
        // search term can be any (no need to re-check refurl != url)
        {
            stepLog = sessionLog.addStepLog (GeneratorConstants.TRAFFIC_STEPID_SEARCH_TERM, logTime);
            stepLog.setUserSegment (userRecord.getSegment());
            selectedRefinedSearchTerm = this.curatedSearchTerms.selectSearchTermAtRandom ();
            stepResult = this.stepsHandler.handleStepSearchTerm (stepResult, userRecord, logTime, stepLog, selectedRefinedSearchTerm.getInitialQuery ());
            if (stepResult instanceof StepResultInvalidData) {
                MessageLogger.logWarning ((String) stepResult.getData ());
                stepLog.setStepResult ((String) stepResult.getData ());
                return stepResult;
    ...
    private StepResult startZeroResult2SearchRefinedQuerySession (StepResult prevStepResult,
        ...
        // 'regular' (non-zero-result) term
        {
            stepLog = sessionLog.addStepLog (GeneratorConstants.TRAFFIC_STEPID_SEARCH_TERM, logTime);
            stepLog.setUserSegment (userRecord.getSegment());
            selectedSearchTerm = this.curatedSearchTerms.selectSearchTermAtRandom ();
            stepResult = this.stepsHandler.handleStepSearchTerm (stepResult, userRecord, logTime, stepLog, selectedSearchTerm.getInitialQuery ());
            if (stepResult instanceof StepResultInvalidData) {
                MessageLogger.logWarning ((String) stepResult.getData ());
                stepLog.setStepResult ((String) stepResult.getData ());
                return stepResult;
        ...
        switch (journeyData.getCustomJourneyType()) {
            case GeneratorConstants.CUSTOM_JOURNEY_TYPE_LPC:    
                ...
                if (rand < 90) {
                    ...
                    selectedSearchTerm = this.curatedSearchTerms.selectSearchTermAtRandom (prevStepResult.getUrl());
                    searchTerm = selectedSearchTerm.getInitialQuery ();
                }
    ...
    private ArrayList <Integer> prepareWeightedSessionList () {
        ...
        for (int sessionType = 0; sessionType < GeneratorConstants.MAX_SESSION_TYPES; sessionType++) {
            switch (sessionType) {
                case GeneratorConstants.TERM_SEARCH_SESSION:
                    weight = CURATEDTERM_SEARCH_SESSION_WEIGHT;
                    break;
                ...
                default:
                    MessageLogger.logError (String.format ("Unknown session type in curatedJourney: %d", sessionType));
                    weight = -1;
                    break;
            } // end of switch
