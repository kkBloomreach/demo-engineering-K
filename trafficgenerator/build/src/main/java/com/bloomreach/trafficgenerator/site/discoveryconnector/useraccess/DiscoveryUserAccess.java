// IMPORTANT: This class must be thread safe. Same object is used by all threads
// This class is used for per-user-access to discovery via search/suggest/widget/pixel API calls
// The search api code in this class is similar (but not exactly same) as that in the non-user-access class
...
public class DiscoveryUserAccess {
    ...
    public void init () throws Exception {
        if ((this.region.equals (GeneratorConstants.REGION_US) == false) &&
            ...
            throw new Exception (String.format ("DiscoveryUserAccess incorrect region: %s", this.region)); 
        }

        if ((this.realm.equals (GeneratorConstants.REALM_STAGING) == false) &&
            (this.realm.equals (GeneratorConstants.REALM_PROD) == false)) {
            throw new Exception (String.format ("DiscoveryUserAccess incorrect realm: %s", this.realm)); 
        }
    ...
    public void dispatchPixel (PixelBRData pixelData) throws Exception {
        ...
        try {
            ...
            e.printStackTrace ();
            throw new Exception (String.format ("Pixel DiscoveryUserAccess exception: %s", e.getMessage ()));
        }
    }
    ...
    public SuggestApiResponse getSuggestApiResponse (ApiBRData apiData)  throws Exception {
        ...
        try {
            ...
            MessageLogger.logError (String.format ("SuggestApi DiscoveryUserAccess exception: %s, queryTerm = %s",
                                                   e.getMessage (), apiData.getParam (ApiBRData.PARAMNAME_Q)));
            throw new Exception (String.format ("SuggestApi DiscoveryUserAccess exception: %s, queryTerm = %s",
                                                 e.getMessage (), apiData.getParam (ApiBRData.PARAMNAME_Q)));
        }
    ...
    public WidgetApiResponse getWidgetApiResponse (ApiBRData apiData, String widgetCode)  throws Exception {
        ...
        try {
            ...
            exceptionMsg = String.format ("WidgetApi DiscoveryUserAccess exception: widgetId %s, msg = %s", widgetId, e.getMessage ());
            MessageLogger.logError (exceptionMsg);
            throw new Exception (exceptionMsg);
        }
    ...
    private void dispatchPixelApi (String endPoint, 
        ...
        MessageLogger.logDebug (String.format ("Pixel DiscoveryUserAccess serverUrl %s\n", URLDecoder.decode (serverUrlStr, "UTF-8")));
        ...
        MessageLogger.logDebug ("Pixel DiscoveryUserAccess waiting for response...");
        responseCode = conn.getResponseCode ();
        MessageLogger.logDebug (String.format ("Pixel DiscoveryUserAccess received response code %s\n", responseCode));
        if (responseCode != 200) {
            MessageLogger.logError (String.format ("Pixel DiscoveryUserAccess received bad response code %s\n", responseCode));
            conn.disconnect ();
            return;
        }

        conn.disconnect ();
        MessageLogger.logDebug ("Pixel DiscoveryUserAccess - done"); 
        return;
    }
    ...
    private ParsedQueryApiResponse parseQueryApiResponse (JSONObject receivedJson) throws Exception {
        ...
        for (int i = 0; i < receivedResponseDocs.length (); i++) {
            ...
            if (receivedDoc.has ("style")) 
                searchApiResponseDoc.setStyle (receivedDoc.optString ("style")); // value may be null
    ...
    private SuggestApiResponse dispatchSuggestApi (String endPoint, 
        ...
        MessageLogger.logDebug (String.format ("Suggest api DiscoveryUserAccess waiting for response, serverUrl %s\n", 
                                                    URLDecoder.decode (serverUrlStr,"UTF-8")));
        responseCode = conn.getResponseCode ();
        if (responseCode != 200) {
            MessageLogger.logError (String.format ("Suggest api DiscoveryUserAccess received bad response code %s\n", responseCode));
            conn.disconnect ();
            return suggestApiResponse;
        }
        ...
        conn.disconnect ();
        MessageLogger.logDebug (String.format ("Suggest api DiscoveryUserAccess received suggest response, done"));
        return suggestApiResponse;
    }
...
//         conn.setRequestProperty ("User-Agent", userAgent);
// 
//         MessageLogger.logDebug (String.format ("WidgetApi DiscoveryUserAccess waiting for response, serverUrl %s\n", 
//                                                 URLDecoder.decode (serverUrlStr)));
//         responseCode = conn.getResponseCode ();
//         if (responseCode != 200) {
//             conn.disconnect ();
//             throw new Exception (String.format ("WidgetApi DiscoveryUserAccess received bad response code %s\n", responseCode));
//         }
// 
//         // prepare JSONObject using API response data
//         response = conn.getInputStream ();
