...
class UpdateMain ():
    ...
    def loadEnglishSourceFeed (self):
        srcFeedHandler = jfr.JsonlFeedReader ()
        srcProducts = srcFeedHandler.readSourceFeed (uc.FILENAME_PHG2_ENGLISH_JSONL_SOURCE_FEED_IN) 
        return srcProducts

    def loadGermanSourceFeed (self):
        srcFeedHandler = jfr.JsonlFeedReader ()
        srcProducts = srcFeedHandler.readSourceFeed (uc.FILENAME_PHG_DE_JSONL_SOURCE_FEED_IN) 
        return srcProducts
    ...
    def performUpdates (self, revision_handler, english_src_products, german_src_products, inject_av_map):
        try:
            revision_handler.set_english_source_records (english_src_products)
            revision_handler.set_german_source_records (german_src_products)
    ...
    def writeJsonlFeed (self, updatedProducts):
        ...
        feedWriter.write (uc.FILENAME_PHG2_DE_UPDATED_JSONL_FEED_OUT)
        return
 
    def writeAttributesToFile (self, attributeList):
        # sort the list
        attributeList.sort ()
        savePath = uc.FILENAME_PHG2_DE_UPDATED_FEED_ATTRIBUTELIST_OUT
    ...
    def writeTabularFeed (self, tabularRecords):
        ...
        feedWriter.writeTSVFeed (uc.FILENAME_PHG2_DE_UPDATED_TSV_FEED_OUT)
        return

    def writeInjectedAVMap (self, injected_av_map):
        output_path = uc.FILENAME_PHG2_DE_INJECTED_AVMAP_OUT
...
if __name__ == '__main__':
    ...
    # read feed file - ph_global_2 english (jsonl)
    englishSourceProducts = updateDriver.loadEnglishSourceFeed ()

    # read feed file - ph_global german (jsonl)
    germanSourceProducts = updateDriver.loadGermanSourceFeed ()

    # read inject av map
    inject_av_map = updateDriver.read_inject_av_map ()

    # process the feed (ie, 'update')
    updatedProducts, updatedAttributeList = updateDriver.performUpdates (revision_handler, englishSourceProducts, germanSourceProducts, inject_av_map)
