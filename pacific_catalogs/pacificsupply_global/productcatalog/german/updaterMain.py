...
class UpdateMain ():
    ...
    def loadSourceFeed (self):
        ...
        srcProducts = srcFeedHandler.readSourceFeed (uc.FILENAME_PSG_ENGLISH_JSONL_SOURCE_FEED_IN) 
        return srcProducts
    ...
    def perform_revision (self, revision_handler, src_products, inject_av_map):
        try:
            revision_handler.set_source_records (src_products)
            revision_handler.set_inject_av_map (inject_av_map)
            updatedProducts, updatedProductsDatahub, updatedAttributeList = revision_handler.perform_revision ()
            return updatedProducts, updatedProductsDatahub, updatedAttributeList
    ...
    def writeJsonlFeed (self, updatedProducts, format = 'dataconnect'):
        ...
        if format == 'dataconnect':
            feedWriter.write (uc.FILENAME_PSG_DE_SUBSET_JSONL_FEED_OUT)
        elif format == 'datahub':
            feedWriter.write (uc.FILENAME_PSG_DE_SUBSET_DATAHUB_JSONL_FEED_OUT)
        else:
    ...
    def writeAttributesToFile (self, attributeList):
        # sort the list
        attributeList.sort ()
        savePath = uc.FILENAME_PSG_DE_SUBSET_FEED_ATTRIBUTELIST_OUT
    ...
    def writeTabularFeed (self, tabularRecords):
        ...
        feedWriter.writeTSVFeed (uc.FILENAME_PSG_DE_SUBSET_TSV_FEED_OUT)
        return

    def writeInjectedAVMap (self, injected_av_map):
        output_path = uc.FILENAME_PSG_DE_SUBSET_INJECTED_AVMAP_OUT
...
if __name__ == '__main__':
    ...
    updatedProducts, updatedProductsDatahub, updatedAttributeList = updateDriver.perform_revision (revision_handler, sourceProducts, inject_av_map)
    ...
    if (updatedProducts != None) and (len (updatedProducts) > 0):
        ...
        # build tabular feed, then save as .tsv and .csv - not needed in this case
        ...
        updateDriver.writeJsonlFeed (updatedProductsDatahub, format = 'datahub')
        updateDriver.writeAttributesToFile (updatedAttributeList)

        # currently, tabular feed is created only for 'full' feed -- not needed in this case
