...
class Revision (RevisionBase) :
    ...
    def _initialize (self, source_records, inject_av_map):
        ...
        if os.path.exists (rc.FILENAME_CATID_NAME_MAP_TSV_IN):
            with open (rc.FILENAME_CATID_NAME_MAP_TSV_IN, 'r') as input:
        ...
        if os.path.exists (rc.FILENAME_ATTRIBUTE_TRANSLATION_MAP_TSV_IN):
            self._attribute_translation_map = []
            with open (rc.FILENAME_ATTRIBUTE_TRANSLATION_MAP_TSV_IN, 'r') as input:
                tsv_reader = csv.DictReader (input, delimiter = '\t')
                for row in tsv_reader:
    ...
    def _perform_update_internal (self, record):
        ...
        for attrib in rc.TRANSLATED_ATTRIBUTES_LIST:
