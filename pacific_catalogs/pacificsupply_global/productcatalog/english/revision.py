...
# -- Version0 of PacificSupply GLOBAL English catalog
...
class Revision (RevisionBase) :
    ...
    def _initialize (self, source_records, inject_av_map):
        ...
        if os.path.exists (rc.FILENAME_SELECTED_CATIDLIST_TSV_IN):
            with open (rc.FILENAME_SELECTED_CATIDLIST_TSV_IN, 'r') as input:
        ...
        if os.path.exists (rc.FILENAME_ADDITIONAL_PIDLIST_TSV_IN):
            with open (rc.FILENAME_ADDITIONAL_PIDLIST_TSV_IN, 'r') as input:
                tsv_reader = csv.DictReader (input, delimiter = '\t')
                for row in tsv_reader:
    ...
    def _finalize (self, updated_products):
        ...
        with open (rc.FILENAME_CATID_NAME_MAP_TSV_OUT, 'w') as file_output:
    ...
    def _perform_update_internal (self, record):
        ...
        for attrib in record ['value']['attributes']:
            if attrib in rc.SELECTED_ATTRIBUTES_LIST:
                continue
            else:
                try:
