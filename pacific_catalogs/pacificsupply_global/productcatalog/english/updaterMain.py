...
class UpdateMain ():
    ...
    def loadSourceFeed (self):
        ...
        srcProducts = srcFeedHandler.readSourceFeed (uc.FILENAME_PS_ENGLISH_JSONL_SOURCE_FEED_IN) 
        return srcProducts
    ...
    def writeJsonlFeed (self, updatedProducts, format = 'dataconnect'):
        ...
        if format == 'dataconnect':
            feedWriter.write (uc.FILENAME_PSG_ENGLISH_SUBSET_JSONL_FEED_OUT)
        elif format == 'datahub':
            feedWriter.write (uc.FILENAME_PSG_ENGLISH_SUBSET_DATAHUB_JSONL_FEED_OUT)
        else:
    ...
    def writeAttributesToFile (self, attributeList):
        # sort the list
        attributeList.sort ()
        savePath = uc.FILENAME_PSG_ENGLISH_SUBSET_FEED_ATTRIBUTELIST_OUT
    ...
    def writeTabularFeed (self, tabularRecords):
        ...
        feedWriter.writeTSVFeed (uc.FILENAME_PSG_ENGLISH_SUBSET_TSV_FEED_OUT)
        return

    def writeInjectedAVMap (self, injected_av_map):
        output_path = uc.FILENAME_PSG_ENGLISH_SUBSET_INJECTED_AVMAP_OUT
