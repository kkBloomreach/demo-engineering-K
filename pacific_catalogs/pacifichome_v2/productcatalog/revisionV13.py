# V13 changes
# -- first do data-cleaning (ie, remove selected color values from title, 
# -- replace those in description.
# -- Also clean up some terms spelledspelled differently in different products (eg, "Micro Suede")
# -- is same as "Microsuede". After such clean up, drop products that have same title
# -- (case insensitive)
# -- Then generate 2 to 4 variants for each product in select categories
# -- also remove some old unused attributes -- 'bread_crumb', 'bread_crumbid', 'int_pid', 'int_skuid'
...
# eg: AWS_S3_IMAGES_FOLDER/<pillows>/variants/<variant_image>
LIVINGROOM_VARIANTS_IMAGE_SUBFOLDER = 'livingroom' 
...
class RevisionV13 (RevisionBase) :
    def __init__ (self):
        ...
        # {"term, replace}
        self._adjust_misspelled_terms_list = []
    ...
    def _initialize (self, source_records, inject_av_map):
        ...
        self._adjust_misspelled_terms_list = self._read_adjust_misspelled_terms_list ()
        if (self._adjust_misspelled_terms_list == None):
            logging.debug ("No title terms to adjust")
        return True
    ...
    def _perform_record_update (self, record):
        ...
        if self._is_select_category (record) == True:
            logging.debug ('procesing record pid: %s' % pid)
            updated_record = self._perform_update_internal (record, inject_av_record)
        else:
            updated_record = record

        # remove old (unused) attributes. If duplicate title, updated_product is None
    ...
    def _read_adjust_color_name_list (self):
        logging.info ("reading source: %s" % rcv13.FILENAME_ADJUST_COLOR_NAMES_TSV_IN)
        adjust_color_names = []
        if os.path.exists (rcv13.FILENAME_ADJUST_COLOR_NAMES_TSV_IN):
            with open (rcv13.FILENAME_ADJUST_COLOR_NAMES_TSV_IN, 'r') as file_obj:
            ...
            logging.error ('cannot find source: %s' % rcv13.FILENAME_ADJUST_COLOR_NAMES_TSV_IN)
            adjust_color_names = None
        return adjust_color_names

    def _read_adjust_misspelled_terms_list (self):
        logging.info ("reading source: %s" % rcv13.FILENAME_ADJUST_MISSPELLED_TERMS_TSV_IN)
        adjust_misspelled_terms = []
        if os.path.exists (rcv13.FILENAME_ADJUST_MISSPELLED_TERMS_TSV_IN):
            with open (rcv13.FILENAME_ADJUST_MISSPELLED_TERMS_TSV_IN, 'r') as file_obj:
                dict_reader = csv.DictReader (file_obj, delimiter='\t')
                for row in dict_reader:
                    adjust_misspelled_terms.append (row)
                file_obj.close ()
            logging.info ('adjust record count: %s' % len (adjust_misspelled_terms))
        else:
            logging.error ('cannot find source: %s' % rcv13.FILENAME_ADJUST_MISSPELLED_TERMS_TSV_IN)
            adjust_misspelled_terms = None
        return adjust_misspelled_terms

    # 'record' is in select-category
    def _perform_update_internal (self, record, inject_av_record):
        pid = record ['value']['attributes']['pid']

        # adjust title terms if needed
        title = self._adjust_misspelled_terms_if_needed (record ['value']['attributes']['title'])

        # check for color name in title
        title = self._replace_color_in_title (title)
        ...
        # not a duplicate title - continue
        ...
        description = self._adjust_misspelled_terms_if_needed (description)
    ...
    def _adjust_misspelled_terms_if_needed (self, title):
        for adjust_term_record in self._adjust_misspelled_terms_list:
            if title.find (adjust_term_record ['term']) >= 0:
                title = title.replace (adjust_term_record ['term'], adjust_term_record ['replace'])
    ...
    def _generate_variants (self, updated_record):
        ...
        count_variants_to_add = int (random.random () * (rcv13.MAX_VARIANTS_PER_PRODUCT)) + rcv13.MIN_VARIANTS_PER_PRODUCT
        available_colors = rcv13.COLOR_CHOICES.copy ()
        for count in range (0, count_variants_to_add):
            ...
            logging.debug ('variant color = %s' % select_color)
            ...
            # variant_id: <pid>_<count> (EG, 133321_1)
            ...
            for this_variant_num in range (0, count_variants_to_add):
                if this_variant_num != default_variant_num:  # variant other-than-default
                    ...
                    if product_has_discounted_variants:
                        ...
                        discount_factor = 1 - (random.random () * 0.13)  # variant price = default_price - {0 -> 0.2}
    ...
    # variant_id = <pid>_<num> (EG, 133213_1)
    def _generate_variant_image (self, updated_record, variant_id, variant_color):
        ...
        local_webp_image_path = '%s/%s.webp' % (rcv13.THUMB_IMAGE_LOCAL_DIR, file_base_name)
        ...
        if (os.path.exists (local_webp_image_path) == False):
            ...
            if (img_bytes != None) and (len (img_bytes) > 0):
                local_image_store_path_png = '%s/%s.png' % (rcv13.THUMB_IMAGE_LOCAL_DIR, file_base_name)
        ...
        if local_webp_image_path != None:
            ...
            swatch_img_url = '%s/%s/variants/%s' % (rcv13.THUMB_IMAGE_URL_PROLOG, 
                                                             LIVINGROOM_VARIANTS_IMAGE_SUBFOLDER,
                                                             s3_file_name)
    ...
    def _generate_variant_image_bytes (self, updated_record, select_color):
        ...
        openai.api_key = rcv13.OPENAI_KEY
        try:
            openai_response = openai.images.generate (
                 model = rcv13.OPENAI_MODEL_IMAGE_GENERATION,
                 prompt = img_prompt,
                 n = 1,
                 size = '1024x1024',
                 quality = 'high',
    ...
    def _construct_s3_cp_command (self, img_upload_record):
        ...
        s3_path = '%s/%s/variants/%s' % (rcv13.AWS_S3_IMAGES_FOLDER, 
                                         LIVINGROOM_VARIANTS_IMAGE_SUBFOLDER, 
                                         s3_file_name) 
    ...
    # eg, aws --profile bloomreach-demo_main s3 rm s3://pacific-demo-data.bloomreach.cloud/home/images/webp/gen/<category>/variants/file_name
...
if __name__ == '__main__':
    ...
    rv = RevisionV13 ()
    logging.info ('RevisionV13 finish...')
