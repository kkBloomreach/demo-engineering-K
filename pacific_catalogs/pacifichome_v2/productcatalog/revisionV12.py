...
class RevisionV12 (RevisionBase) :
    ...
    def _read_adjust_color_name_list (self):
        logging.info ("reading source: %s" % rcv12.FILENAME_ADJUST_COLOR_NAMES_TSV_IN)
        adjust_color_names = []
        if os.path.exists (rcv12.FILENAME_ADJUST_COLOR_NAMES_TSV_IN):
            with open (rcv12.FILENAME_ADJUST_COLOR_NAMES_TSV_IN, 'r') as file_obj:
            ...
            logging.error ('cannot find source: %s' % rcv12.FILENAME_ADJUST_COLOR_NAMES_TSV_IN)
            adjust_color_names = None
        return adjust_color_names

    def _read_adjust_misspelled_terms_list (self):
        logging.info ("reading source: %s" % rcv12.FILENAME_ADJUST_MISSPELLED_TERMS_TSV_IN)
        adjust_misspelled_terms = []
        if os.path.exists (rcv12.FILENAME_ADJUST_MISSPELLED_TERMS_TSV_IN):
            with open (rcv12.FILENAME_ADJUST_MISSPELLED_TERMS_TSV_IN, 'r') as file_obj:
                dict_reader = csv.DictReader (file_obj, delimiter='\t')
                for row in dict_reader:
            ...
            logging.error ('cannot find source: %s' % rcv12.FILENAME_ADJUST_MISSPELLED_TERMS_TSV_IN)
    ...
    def _generate_variants (self, updated_record):
        ...
        count_variants_to_add = int (random.random () * (rcv12.MAX_VARIANTS_PER_PRODUCT)) + rcv12.MIN_VARIANTS_PER_PRODUCT
        available_colors = rcv12.COLOR_CHOICES.copy ()
    ...
    def _select_uniq_color (self, color_list):
        color_indx = int (random.random () * (len (rcv12.COLOR_CHOICES)))
        while True:
            select_color = rcv12.COLOR_CHOICES [color_indx]
    ...
    def _generate_variant_image (self, updated_record, variant_id, variant_color):
        ...
        local_webp_image_path = '%s/%s.webp' % (rcv12.THUMB_IMAGE_LOCAL_DIR, file_base_name)
        ...
        if (os.path.exists (local_webp_image_path) == False):
            ...
            if (img_bytes != None) and (len (img_bytes) > 0):
                local_image_store_path_png = '%s/%s.png' % (rcv12.THUMB_IMAGE_LOCAL_DIR, file_base_name)
        ...
        if local_webp_image_path != None:
            ...
            swatch_img_url = '%s/%s/variants/%s' % (rcv12.THUMB_IMAGE_URL_PROLOG, 
    ...
    def _generate_variant_image_bytes (self, updated_record, select_color):
        ...
        openai.api_key = rcv12.OPENAI_KEY
        try:
            openai_response = openai.images.generate (
                 model = rcv12.OPENAI_MODEL_IMAGE_GENERATION,
                 prompt = img_prompt,
                 n = 1,
                 size = '1024x1024',
                 quality = 'high',
    ...
    def _construct_s3_cp_command (self, img_upload_record):
        ...
        s3_path = '%s/%s/variants/%s' % (rcv12.AWS_S3_IMAGES_FOLDER, 
                                         PILLOWS_VARIANTS_IMAGE_SUBFOLDER, 
                                         s3_file_name) 
    ...
    def _construct_s3_rm_command (self, img_upload_record):
        ...
        s3_path = '%s/%s/variants/%s' % (rcv12.AWS_S3_IMAGES_FOLDER, 
                                         PILLOWS_VARIANTS_IMAGE_SUBFOLDER, 
                                         s3_file_name)
        aws_rm_command = '%s %s' % (rcv12.AWS_RM_COMMAND_PREAMBLE, s3_path)
...
if __name__ == '__main__':
    ...
    rv = RevisionV12 ()
    logging.info ('RevisionV12 finish...')
