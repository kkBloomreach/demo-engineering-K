...
class RevisionV8 (RevisionBase) :
    ...
    def _generate_thumb_image (self, updated_record):
        ...
        file_base_name = '%s_0_image' % (updated_record ['value']['attributes']['pid'])
        local_webp_image_path = '%s/%s.webp' % (rcv8.THUMB_IMAGE_LOCAL_DIR, file_base_name)
        ...
        if (os.path.exists (local_webp_image_path) == False):
            ...
            if (img_bytes != None) and (len (img_bytes) > 0):
                local_image_store_path_png = '%s/%s.png' % (rcv8.THUMB_IMAGE_LOCAL_DIR, file_base_name)
        ...
        if local_webp_image_path != None:
            thumb_img_url = '%s/%s_0_image.%s' % (rcv8.THUMB_IMAGE_URL_PROLOG, updated_record ['value']['attributes']['pid'], 'webp')
    ...
    def _generate_thumb_image_bytes (self, updated_record):
        ...
        openai.api_key = rcv8.OPENAI_KEY
        try:
            openai_response = openai.images.generate (
                 model = rcv8.OPENAI_MODEL_IMAGE_GENERATION,
                 prompt = img_prompt,
                 n = 1,
                 size = '1024x1024',
                 quality = 'high',
    ...
    def _prepare_upload_script (self):
        upload_count = 0
        upload_script_file = open (rcv8.FILENAME_AWS_UPLOAD_SCRIPT_OUT, 'w')
    ...
    def _construct_s3_cp_command (self, img_upload_record):
        ...
        s3_path = '%s/%s' % (rcv8.AWS_S3_IMAGES_FOLDER, s3_file_name) 
        aws_cp_command = '%s %s %s' % (rcv8.AWS_CP_COMMAND_PREAMBLE, local_image_path, s3_path)
    ...
    def _construct_s3_rm_command (self, img_upload_record):
        ...
        s3_path = '%s/%s' % (rcv8.AWS_S3_IMAGES_FOLDER, s3_file_name)
        aws_rm_command = '%s %s' % (rcv8.AWS_RM_COMMAND_PREAMBLE, s3_path)
...
if __name__ == '__main__':
    ...
    rv = RevisionV8 ()
    logging.info ('RevisionV8 Finish...')
