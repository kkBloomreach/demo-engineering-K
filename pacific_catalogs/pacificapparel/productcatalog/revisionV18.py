# V18 changes
# -- change images of 'Jewellery' products using new openai model and prompt
# -- there are three categories with name "jewellery" (10700, 20700, 90000)
...
import revisionConstantsV18 as rcv18
...
JEWELLERY_IMAGE_SUBFOLDER = 'jewellery' # sub-folder/bucket in s3

class RevisionV18 (RevisionBase) :
    def __init__ (self):
        logging.info ('Perform update, version v18')
        super().__init__ ()
    ...
    def _perform_record_update (self, record):
        ...
        if self._is_jewellery_product (record) == True:
    ...
    # INTERNAL METHODS
    def _is_jewellery_product (self, record):
        ...
        for branch in category_paths:
            ...
            if full_path in JEWELLERY_CATEGORY_IDS:
                return True
        return False
    ...
    def _generate_thumb_image (self, updated_record):
        ...
        local_webp_image_path = '%s/%s.webp' % (rcv18.THUMB_IMAGE_LOCAL_DIR, file_base_name)
        ...
        if (os.path.exists (local_webp_image_path) == False):
            ...
            if (img_bytes != None) and (len (img_bytes) > 0):
                local_image_store_path_png = '%s/%s.png' % (rcv18.THUMB_IMAGE_LOCAL_DIR, file_base_name)
        ...
        if local_webp_image_path != None:
            thumb_img_url = '%s/%s/%s_image.%s' % (rcv18.THUMB_IMAGE_URL_PROLOG, 
                                                   JEWELLERY_IMAGE_SUBFOLDER,
    ...
    def _generate_thumb_image_bytes (self, updated_record):
        ...
        openai.api_key = rcv18.OPENAI_KEY
        try:
            openai_response = openai.images.generate (
                 model = rcv18.OPENAI_MODEL_IMAGE_GENERATION,
                 prompt = img_prompt,
                 n = 1,
                 size = '1024x1024',
                 quality = 'high',
    ...
    def _prepare_upload_script (self):
        upload_count = 0
        upload_script_file = open (rcv18.FILENAME_AWS_UPLOAD_SCRIPT_OUT, 'w')
    ...
    def _construct_s3_cp_command (self, img_upload_record):
        ...
        s3_path = '%s/%s/%s' % (rcv18.AWS_S3_IMAGES_FOLDER, 
                                JEWELLERY_IMAGE_SUBFOLDER,
                                s3_file_name) 
        aws_cp_command = '%s %s %s' % (rcv18.AWS_CP_COMMAND_PREAMBLE, local_image_path, s3_path)
    ...
    def _construct_s3_rm_command (self, img_upload_record):
        ...
        s3_path = '%s/%s/%s' % (rcv18.AWS_S3_IMAGES_FOLDER, 
                                JEWELLERY_IMAGE_SUBFOLDER,
                                s3_file_name)
        aws_rm_command = '%s %s' % (rcv18.AWS_RM_COMMAND_PREAMBLE, s3_path)
...
if __name__ == '__main__':
    ...
    rv = RevisionV18 ()
    logging.info ('RevisionV18 finish...')
