...
# -- this version is exact copy of v26 except for a bug fix
# -- Earlier, we changed swatch-image-url but not sku->color, which caused color != image-color
...
import revisionConstantsV28 as rcv28

# in this revision, products only in these categories are considered for generating variants
SELECT_CATEGORY_IDS = [
    '80000>80100',   # bags > purses
    '80000>80200'    # bags > backpacks and totes
]
...
# eg: AWS_S3_IMAGES_FOLDER/<bags>/variants_new/<variant_image>
BAGS_VARIANTS_IMAGE_SUBFOLDER = 'bags' 

class RevisionV28 (RevisionBase) :
    def __init__ (self):
        logging.info ('Perform update, version v28')
        super().__init__ ()
    ...
    def _perform_update_internal (self, record, inject_av_record):
        ...
        local_default_image_path_webp = '%s/%s' % (rcv28.THUMB_IMAGE_LOCAL_DIR, local_default_image_file_name_webp)
        ...
        available_colors = rcv28.COLOR_CHOICES.copy ()
        ...
        swatch_image_url = '%s/%s/variants_new/%s' % (rcv28.THUMB_IMAGE_URL_PROLOG, 
    ...
    def _handle_preexisting_images (self, updated_record):
        ...
        for variant_id in variant_list.keys():
            ...
            if variant_color_swatchurl_record != None:
                ...
                local_image_path_webp = '%s/%s' % (rcv28.THUMB_IMAGE_LOCAL_DIR, file_base_name)
    ...
    # variant_id = <pid>_<num> (EG, 123283_1)
    ...
    def _generate_default_variant_image_png (self, updated_record, default_variant_id, available_colors):
        ...
        if (img_bytes != None) and (len (img_bytes) > 0):
            ...
            local_image_store_path_png = '%s/%s' % (rcv28.THUMB_IMAGE_LOCAL_DIR, file_base_name_png)
    ...
    def _generate_default_variant_image_bytes (self, updated_record, select_color):
        ...
        openai.api_key = rcv28.OPENAI_KEY
        try:
            openai_response = openai.images.generate (
                 model = rcv28.OPENAI_MODEL_IMAGE_GENERATION,
                 prompt = img_prompt,
                 n = 1,
                 size = '1024x1024',
                 quality = 'high',
    ...
    def _generate_swatch_image_with_different_color (self, variant_id, local_default_image_path_png, available_colors):
        ...
        if img_bytes != None and len (img_bytes) > 0:
            ...
            local_image_path_png  = '%s/%s.png' % (rcv28.THUMB_IMAGE_LOCAL_DIR, variant_id)
            ...
            local_image_path_webp = '%s/%s' % (rcv28.THUMB_IMAGE_LOCAL_DIR, local_image_file_name_webp)
    ...
    def _prepare_upload_script (self):
        upload_count = 0
        upload_script_file = open (rcv28.FILENAME_AWS_UPLOAD_SCRIPT_OUT, 'w')
    ...
    def _construct_s3_cp_command (self, img_upload_record):
        ...
        s3_path = '%s/%s/variants_new/%s' % (rcv28.AWS_S3_GEN_GPTIMAGE_1_IMAGES_FOLDER, 
                                             BAGS_VARIANTS_IMAGE_SUBFOLDER, 
                                             s3_file_name) 
        aws_cp_command = '%s %s %s' % (rcv28.AWS_CP_COMMAND_PREAMBLE, local_image_path, s3_path)
    ...
    def _construct_s3_rm_command (self, img_upload_record):
        ...
        s3_path = '%s/%s/variants_new/%s' % (rcv28.AWS_S3_GEN_GPTIMAGE_1_IMAGES_FOLDER, 
                                             BAGS_VARIANTS_IMAGE_SUBFOLDER, 
                                             s3_file_name)
        aws_rm_command = '%s %s' % (rcv28.AWS_RM_COMMAND_PREAMBLE, s3_path)
    ...
    def _read_variant_color_swatchurl_map (self):
        ...
        if os.path.exists (rcv28.FILENAME_VARIANT_COLOR_SWATCHURL_MAP_TSV_OUT) == False:
            return variant_color_swatchurl_map
        with open (rcv28.FILENAME_VARIANT_COLOR_SWATCHURL_MAP_TSV_OUT, 'r') as input_file:
    ...
    def _save_variant_color_swatchurl_map (self):
        with open (rcv28.FILENAME_VARIANT_COLOR_SWATCHURL_MAP_TSV_OUT, 'w') as output_file:
...
if __name__ == '__main__':
    ...
    rv = RevisionV28 ()
    logging.info ('RevisionV28 finish...')
