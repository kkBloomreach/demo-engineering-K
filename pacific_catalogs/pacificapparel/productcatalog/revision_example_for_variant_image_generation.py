# V33 changes
# -- generate AI images for 'dresses' (catId = 21200)
# -- Use a newer prompt
# -- Also use openAI newer image-generation api version
...
class RevisionV33 (RevisionBase) :
    ...
    def _perform_update_internal (self, record, inject_av_record):
        ...
        # if we have already created image, reuse it
        local_default_image_file_name_webp = '%s_image.webp' % (default_variant_id)
        local_default_image_path_webp = '%s/%s' % (rcv33.THUMB_IMAGE_LOCAL_DIR, local_default_image_file_name_webp)
        ...
        if (local_default_image_path_png == None):
            logging.error ('Could not generate image for pid: %s' % pid)
            return updated_record
        ...
        swatch_image_url = '%s/%s/variants_new/%s' % (rcv33.THUMB_IMAGE_URL_PROLOG, 
    ...
    def _handle_preexisting_images (self, updated_record):
        ...
        for variant_id in variant_list.keys():
            ...
            if variant_color_swatchurl_record != None:
                ...
                local_image_path_webp = '%s/%s' % (rcv33.THUMB_IMAGE_LOCAL_DIR, file_base_name)
    ...
    # variant_id = <pid>_<num> (EG, 123333_1)
    ...
    def _generate_default_variant_image_png (self, updated_record, default_variant_id, available_colors):
        ...
        if (img_bytes != None) and (len (img_bytes) > 0):
            ...
            local_image_store_path_png = '%s/%s' % (rcv33.THUMB_IMAGE_LOCAL_DIR, file_base_name_png)
    ...
    def _generate_swatch_image_with_different_color (self, variant_id, local_default_image_path_png, available_colors):
        ...
        if img_bytes != None and len (img_bytes) > 0:
            ...
            local_image_path_png  = '%s/%s.png' % (rcv33.THUMB_IMAGE_LOCAL_DIR, variant_id)
            ...
            local_image_path_webp = '%s/%s' % (rcv33.THUMB_IMAGE_LOCAL_DIR, local_image_file_name_webp)
    ...
    def _construct_s3_cp_command (self, img_upload_record):
        ...
        s3_path = '%s/%s/variants_new/%s' % (rcv33.AWS_S3_GEN_GPTIMAGE_1_IMAGES_FOLDER, 
                                             SHOES_VARIANTS_IMAGE_SUBFOLDER, 
                                             s3_file_name) 
    ...
    def _read_variant_color_swatchurl_map (self):
        ...
        if os.path.exists (rcv33.FILENAME_VARIANT_COLOR_SWATCHURL_MAP_TSV_OUT) == False:
            return variant_color_swatchurl_map
        with open (rcv33.FILENAME_VARIANT_COLOR_SWATCHURL_MAP_TSV_OUT, 'r') as input_file:
    ...
    def _save_variant_color_swatchurl_map (self):
        with open (rcv33.FILENAME_VARIANT_COLOR_SWATCHURL_MAP_TSV_OUT, 'w') as output_file:
