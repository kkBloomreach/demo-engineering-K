# NOTES:
#1.  This is an example script that was used in a previous revision to 
# -- generate images using openAI 
# -- create a aws-image-upload script for each new image
# --- once that aws-script is created, need to execute it separately (ie, not done by this script itself)
# --- so that if there any upload-errors, they can be resolved separately
#2.  Ignore some other update done in this script (as they were done in that prevision revision).
...
import revisionConstantsV34 as rcv34
...
Use refined lighting and show full face portrait. Image must have crisp details, be clean, modern presentation suitable for high‑end ecommerce brand. The image must show the model's entire body including the ankle. Use a neutral, elegant background that enhances the product without distraction. The style should feel sophisticated, upscale, and consistent with luxury fashion photography. No text, logos, or additional props should appear in the image—only the model wearing the item displayed clearly and attractively. 
"""
...
# specific pids to have 'ankle-length', 'knee-length'
SELECTED_PIDS = [
    {'pid': '110713', 'title_extension': 'ankle-length'},
    {'pid': '110723', 'title_extension': 'ankle-length'},
    {'pid': '110724', 'title_extension': 'knee-length'},
    {'pid': '110725', 'title_extension': 'knee-length'},
    {'pid': '110726', 'title_extension': 'ankle-length'},
    {'pid': '110727', 'title_extension': 'knee-length'}
]

WOMEN_DRESSES_IMAGE_SUBFOLDER = 'dresses' # sub-folder/bucket in s3

class RevisionV34 (RevisionBase) :
    def __init__ (self):
        logging.info ('Perform update, version v34')
        super().__init__ ()
    ...
    def _perform_record_update (self, record):
        ...
        if self._is_select_category (record) == True:
            selected_pid_record = self._is_selected_pid (pid)
            if selected_pid_record != None:
                logging.debug ('Changing image for pid: %s' % pid)
                updated_record = self._perform_update_internal (record, inject_av_record, selected_pid_record)
    ...
    def _is_selected_pid (self, pid):
        for select_record in SELECTED_PIDS:
            if select_record ['pid'] == str (pid):
                return select_record
        return None

    def _perform_update_internal (self, record, inject_av_record, selected_pid_record):
        pid = record ['value']['attributes']['pid']
        ...
        self._generate_thumb_image (updated_record, selected_pid_record ['title_extension'])

        return updated_record

    def _generate_thumb_image (self, updated_record, title_extension):
        # generate new image if previous does not exist
        file_base_name = '%s_image' % (updated_record ['value']['attributes']['pid'])
        local_webp_image_path = '%s/%s.webp' % (rcv34.THUMB_IMAGE_LOCAL_DIR, file_base_name)

        if (os.path.exists (local_webp_image_path) == False):
            img_bytes = self._generate_thumb_image_bytes (updated_record, title_extension)
            if (img_bytes != None) and (len (img_bytes) > 0):
                local_image_store_path_png = '%s/%s.png' % (rcv34.THUMB_IMAGE_LOCAL_DIR, file_base_name)
        ...
        if local_webp_image_path != None:
            thumb_img_url = '%s/%s/%s_image.%s' % (rcv34.THUMB_IMAGE_URL_PROLOG, 
    ...
    def _generate_thumb_image_bytes (self, updated_record, title_extension):
        ...
        title_lower = updated_record ['value']['attributes']['title'].lower ()
        extended_title = title_lower.replace ('dress', ' ' + title_extension + ' ' + 'dress')
        product_details = """
                      Product Name: %s, \n
                      Product Brand: %s, \n
        ...
        openai.api_key = rcv34.OPENAI_KEY
        try:
            openai_response = openai.images.generate (
                 model = rcv34.OPENAI_MODEL_IMAGE_GENERATION,
                 prompt = img_prompt,
                 n = 1,
                 size = '1024x1024',
                 quality = 'high',
    ...
    def _prepare_upload_script (self):
        upload_count = 0
        upload_script_file = open (rcv34.FILENAME_AWS_UPLOAD_SCRIPT_OUT, 'w')
    ...
    def _construct_s3_cp_command (self, img_upload_record):
        ...
        s3_path = '%s/%s/%s' % (rcv34.AWS_S3_GEN_GPTIMAGE_1_IMAGES_FOLDER, 
                                WOMEN_DRESSES_IMAGE_SUBFOLDER,
                                s3_file_name) 
        aws_cp_command = '%s %s %s' % (rcv34.AWS_CP_COMMAND_PREAMBLE, local_image_path, s3_path)
    ...
    def _construct_s3_rm_command (self, img_upload_record):
        ...
        s3_path = '%s/%s/%s' % (rcv34.AWS_S3_GEN_GPTIMAGE_1_IMAGES_FOLDER, 
                                WOMEN_DRESSES_IMAGE_SUBFOLDER,
                                s3_file_name)
        aws_rm_command = '%s %s' % (rcv34.AWS_RM_COMMAND_PREAMBLE, s3_path)
...
if __name__ == '__main__':
    ...
    rv = RevisionV34 ()
    logging.info ('RevisionV34 finish...')
