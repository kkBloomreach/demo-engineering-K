...
# -- this version is exact copy of v25 except for a bug fix
...
import revisionConstantsV27 as rcv27
...
class RevisionV27 (RevisionBase) :
    def __init__ (self):
        logging.info ('Perform update, version v27')
        super().__init__ ()
    ...
    def _perform_update_internal (self, record, inject_av_record):
        ...
        local_default_image_path_webp = '%s/%s' % (rcv27.THUMB_IMAGE_LOCAL_DIR, local_default_image_file_name_webp)
        ...
        available_colors = rcv27.COLOR_CHOICES.copy ()
        ...
        swatch_image_url = '%s/%s/variants_new/%s' % (rcv27.THUMB_IMAGE_URL_PROLOG, 
    ...
    def _handle_preexisting_images (self, updated_record):
        ...
        for variant_id in variant_list.keys():
            ...
            if variant_color_swatchurl_record != None:
                ...
                local_image_path_webp = '%s/%s' % (rcv27.THUMB_IMAGE_LOCAL_DIR, file_base_name)
    ...
    # variant_id = <pid>_<num> (EG, 123273_1)
    ...
    def _generate_default_variant_image_png (self, updated_record, default_variant_id, available_colors):
        ...
        if (img_bytes != None) and (len (img_bytes) > 0):
            ...
            local_image_store_path_png = '%s/%s' % (rcv27.THUMB_IMAGE_LOCAL_DIR, file_base_name_png)
    ...
    def _generate_default_variant_image_bytes (self, updated_record, select_color):
        ...
        openai.api_key = rcv27.OPENAI_KEY
        try:
            openai_response = openai.images.generate (
                 model = rcv27.OPENAI_MODEL_IMAGE_GENERATION,
                 prompt = img_prompt,
                 n = 1,
                 size = '1024x1024',
                 quality = 'high',
    ...
    def _generate_swatch_image_with_different_color (self, variant_id, local_default_image_path_png, available_colors):
        ...
        if img_bytes != None and len (img_bytes) > 0:
            ...
            local_image_path_png  = '%s/%s.png' % (rcv27.THUMB_IMAGE_LOCAL_DIR, variant_id)
            ...
            local_image_path_webp = '%s/%s' % (rcv27.THUMB_IMAGE_LOCAL_DIR, local_image_file_name_webp)
    ...
    def _prepare_upload_script (self):
        upload_count = 0
        upload_script_file = open (rcv27.FILENAME_AWS_UPLOAD_SCRIPT_OUT, 'w')
    ...
    def _construct_s3_cp_command (self, img_upload_record):
        ...
        s3_path = '%s/%s/variants_new/%s' % (rcv27.AWS_S3_GEN_GPTIMAGE_1_IMAGES_FOLDER, 
                                             SHOES_VARIANTS_IMAGE_SUBFOLDER, 
                                             s3_file_name) 
        aws_cp_command = '%s %s %s' % (rcv27.AWS_CP_COMMAND_PREAMBLE, local_image_path, s3_path)
    ...
    def _construct_s3_rm_command (self, img_upload_record):
        ...
        s3_path = '%s/%s/variants_new/%s' % (rcv27.AWS_S3_GEN_GPTIMAGE_1_IMAGES_FOLDER, 
                                             SHOES_VARIANTS_IMAGE_SUBFOLDER, 
                                             s3_file_name)
        aws_rm_command = '%s %s' % (rcv27.AWS_RM_COMMAND_PREAMBLE, s3_path)
    ...
    def _read_variant_color_swatchurl_map (self):
        ...
        if os.path.exists (rcv27.FILENAME_VARIANT_COLOR_SWATCHURL_MAP_TSV_OUT) == False:
            return variant_color_swatchurl_map
        with open (rcv27.FILENAME_VARIANT_COLOR_SWATCHURL_MAP_TSV_OUT, 'r') as input_file:
    ...
    def _save_variant_color_swatchurl_map (self):
        with open (rcv27.FILENAME_VARIANT_COLOR_SWATCHURL_MAP_TSV_OUT, 'w') as output_file:
...
if __name__ == '__main__':
    ...
    rv = RevisionV27 ()
    logging.info ('RevisionV27 finish...')
