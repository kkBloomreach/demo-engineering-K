...
import revisionConstantsV26 as rcv26
...
class RevisionV26 (RevisionBase) :
    def __init__ (self):
        logging.info ('Perform update, version v26')
        super().__init__ ()
    ...
    def _perform_update_internal (self, record, inject_av_record):
        ...
        if default_variant_id == None or default_swatch_image_url == None:
            logging.warning ('Product has no variants: %s' %  pid)
            return record   # this product has no variants -- should not happen

        updated_record = copy.deepcopy (record)
        available_colors = rcv26.COLOR_CHOICES.copy ()
        ...
        local_default_image_path_webp = '%s/%s' % (rcv26.THUMB_IMAGE_LOCAL_DIR, local_default_image_file_name_webp)
        ...
        if load_status == uc.IMAGE_LOADER_STATUS_SUCCESS:
            ...
            local_default_image_path_png = '%s/%s.png' % (rcv26.THUMB_IMAGE_LOCAL_DIR, default_variant_id)
        ...
        for variant_id in variant_list.keys():
            ...
            if variant_id == default_variant_id:
                ...
                swatch_image_url = '%s/%s/variants_new/%s' % (rcv26.THUMB_IMAGE_URL_PROLOG, 
                ...
                if local_image_new_path_webp != None:
                    ...
                    # new variant-image url to set in catalog
                    new_variant_swatch_image_url = '%s/%s/variants_new/%s' % (rcv26.THUMB_IMAGE_URL_PROLOG, 
                                                                              BAGS_VARIANTS_IMAGE_SUBFOLDER,
                                                                              s3_file_name)
    ...
    def _generate_swatch_image_with_different_color (self, variant_id, local_default_image_path_png, available_colors):
        ...
        local_image_new_path_webp = '%s/%s' % (rcv26.THUMB_IMAGE_LOCAL_DIR, local_image_file_name_new_webp)
        if os.path.exists (local_image_new_path_webp) == False:
            ...
            if img_bytes != None and len (img_bytes) > 0:
                ...
                local_image_new_path_png  = '%s/%s_new.png' % (rcv26.THUMB_IMAGE_LOCAL_DIR, variant_id)
    ...
    def _prepare_upload_script (self):
        upload_count = 0
        upload_script_file = open (rcv26.FILENAME_AWS_UPLOAD_SCRIPT_OUT, 'w')
    ...
    def _construct_s3_cp_command (self, img_upload_record):
        ...
        s3_path = '%s/%s/variants_new/%s' % (rcv26.AWS_S3_GEN_GPTIMAGE_1_IMAGES_FOLDER, 
                                             BAGS_VARIANTS_IMAGE_SUBFOLDER, 
                                             s3_file_name) 
        aws_cp_command = '%s %s %s' % (rcv26.AWS_CP_COMMAND_PREAMBLE, local_image_path, s3_path)
    ...
    def _construct_s3_rm_command (self, img_upload_record):
        ...
        s3_path = '%s/%s/variants_new/%s' % (rcv26.AWS_S3_GEN_GPTIMAGE_1_IMAGES_FOLDER, 
                                             BAGS_VARIANTS_IMAGE_SUBFOLDER, 
                                             s3_file_name)
        aws_rm_command = '%s %s' % (rcv26.AWS_RM_COMMAND_PREAMBLE, s3_path)
...
if __name__ == '__main__':
    ...
    rv = RevisionV26 ()
    logging.info ('RevisionV26 finish...')
