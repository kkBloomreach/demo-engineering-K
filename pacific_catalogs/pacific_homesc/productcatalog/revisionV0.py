# V0 changes
# -- generate catalog with reduced product set
...
class RevisionV0 (RevisionBase) :
    ...
    def _initialize (self, source_records, inject_av_map):
        if os.path.exists (rcv0.FILE_NAME_PH3_TARGET_CATEGORY_MAP_INPUT_TSV):
            target_category_map = self._read_target_category_map (rcv0.FILE_NAME_PH3_TARGET_CATEGORY_MAP_INPUT_TSV)
        else:
            logging.error ('Cannot find target PH3 category map: %s' % rcv0.FILE_NAME_PH3_TARGET_CATEGORY_MAP_INPUT_TSV)
            return False
    ...
    def _finalize (self, updated_products):
        ...
        for record in updated_products:
            ...
            if (len (category_paths) == 1):
                ...
                if (cat_pid_count > rcv0.MAX_PRODUCTS_IN_EACH_CATEGORY):
                    # randomly decide if this pid should/should-not be in catalog
                    rand = random.random () * cat_pid_count
                    if rand > rcv0.MAX_PRODUCTS_IN_EACH_CATEGORY:
