...
class RevisionV3 (RevisionBase) :
    ...
    def _perform_update_internal (self, record, pid, inject_av_record):
        ...
        ph2_s3_path = '%s%s%s' % (rcv3.PH2_IMAGE_S3_BUCKET_PREAMBLE, pid, '_0_image.webp')
        phsc_s3_path = '%s%s%s' % (rcv3.PHSC_IMAGE_S3_BUCKET_PREAMBLE, pid, '_0_image.webp')
        local_download_path = '%s/%s%s' % (rcv3.THUMB_IMAGE_LOCAL_DIR, pid, '_0_image.webp')
    ...
    def _adjust_image_urls (self, updated_record, pid):
        img_url = '%s%s%s' % (rcv3.THUMB_IMAGE_URL_PROLOG, pid, '_0_image.webp')
    ...
    def _prepare_download_upload_script (self):
        upload_count = 0
        download_upload_script_file = open (rcv3.FILENAME_AWS_DOWNLOAD_UPLOAD_SCRIPT_OUT, 'w')
        ...
        for img_download_upload_record in self._aws_image_download_upload_records:
            ...
            bash_if_condition = rcv3.BASH_TEMPLATE_IF_FILE_EXISTS % img_download_upload_record ['local_image_path']
            bash_cmd = '%s\n\t%s\n\t%s\n%s\n\t%s\n%s\n' % (bash_if_condition, 
                                                         aws_s3_rm_command, 
    ...
    def _construct_s3_download_command (self, img_download_upload_record):
        ...
        aws_download_command = '%s %s %s' % (rcv3.AWS_CP_COMMAND_PREAMBLE, ph2_s3_path, local_image_path)
    ...
    def _construct_s3_rm_command (self, img_download_upload_record):
        ...
        aws_rm_command = '%s %s' % (rcv3.AWS_RM_COMMAND_PREAMBLE, phsc_s3_path)
    ...
    def _construct_s3_upload_command (self, img_download_upload_record):
        ...
        aws_upload_command = '%s %s %s' % (rcv3.AWS_CP_COMMAND_PREAMBLE, local_image_path, phsc_s3_path)
...
'''
        ...
        s3_path = '%s/%s' % (rcv3.PH2_IMAGE_S3_BUCKET_PREAMBLE, s3_file_name) 
        s3_file_name = '%s_0_image.%s' % (pid, extension)  # .../pid_0_image.<extension>
        s3_path = '%s/%s' % (rcv3.PHSC_IMAGE_S3_BUCKET_PREAMBLE, s3_file_name) 
