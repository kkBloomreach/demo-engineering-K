...
class UpdateMain ():
    ...
    def perform_revision (self, revision_handler, documents_api_response, inject_av_map):
        try:
            revision_handler.set_site_documents_api_response (documents_api_response)
            revision_handler.set_inject_av_map (inject_av_map)
            updated_products, updated_attribute_list = revision_handler.perform_revision ()
            return updated_products, updated_attribute_list
        except Exception as e:
            logging.error ('Error in processing api response: %s', str(e))
        return None, None

    def prepare_injected_av_map (self, updated_records):
        injected_av_map = []
...
if __name__ == '__main__':
    ...
    documents_api_response = updateDriver.getSiteApiResponse (uc.SITE_CONTENT_DOCUMENTS_API_ENDPOINT)

    # read inject av map
    inject_av_map = updateDriver.read_inject_av_map ()

    # process the feed (ie, 'update')
    discovery_records, updated_attribute_list = updateDriver.perform_revision (revision_handler, documents_api_response, inject_av_map)
    ...
    if (discovery_records != None) and (len (discovery_records) > 0):
        ...
        tabularRecords = updateDriver.generateTabulerFeed (discovery_records, updated_attribute_list)

        updateDriver.writeJsonlFeed (discovery_records)
        updateDriver.writeAttributesToFile (updated_attribute_list)
        ...
        logging.warning ('Updated record count is zero')
